...
public class ThreadGraphic2 extends JFrame { //JFrame 상속받아서 변수명을 바로 선언.
											 //변수형을 따로 선언할 필요가 없다.
	
	JLabel count, image, day; //화면에 띄울 count, 이미지, 날짜
	
	public ThreadGraphic2() { //내부(inner)클래스를 사용함으로써 변수 등 코드의 복잡도를 줄인다.
		...
		getContentPane().add(count, BorderLayout.WEST);//글자를 왼쪽에 배치.
		
		day = new JLabel("day");
		day.setForeground(Color.WHITE);
		day.setFont(new Font("굴림", Font.BOLD, 32));
		getContentPane().add(day, BorderLayout.SOUTH);//날짜는 아래쪽에 배치.
		
		image = new JLabel();
		getContentPane().add(image, BorderLayout.CENTER);//이미지는 센터에 보이도록.
		
		
		count c = new count();//동시 실행을 위해 아래에 각 함수를 스레드로 상속받아서 사용.
	...
	public static void main(String[] args) {
		JFrame f = new JFrame("스레드 사용"); //JFrame 선언
		ThreadGraphic2 t = new ThreadGraphic2(); //JFrame을 상속받아 그 안에 스레드를 사용하여 동시 실행이 되게 한다.
	}
	
	public class image extends Thread { //이미지.java 외부 클래스를 재정의
		@Override
		public void run() {
	...
	public class count extends Thread {//카운트.java 외부 클래스 상속
		@Override//재정의
		public void run() {
	...
	public class day extends Thread {//이미지.java 외부 클래스 상속
		@Override
		public void run() {
